...
class SceneManager:
    ...
    def build_fmm_speed(
        ...
        if safe_radius <= robot_radius:
            raise ValueError('safe_radius must be greater than robot_radius')

        d = np.clip(tsdf, 0.0, safe_radius)
        alpha = np.clip((d - robot_radius) / (safe_radius - robot_radius), 0.0, 1.0)

        speed = slow_speed + (1.0 - slow_speed) * cls._smoothstep(alpha)
        speed = np.where(d <= robot_radius, slow_speed, speed)
        speed = np.where(d >= safe_radius, 1.0, speed)
        speed = speed.astype(np.float32, copy=False)
        speed[occupancy == 1] = 0.0
        return speed
    ...
    def fmm_3d(
        ...
        tsdf = np.clip(dist_outside, 0.0, safe_radius)

        # smoothly reduce the wave propagation speed near obstacles
        speed = self.build_fmm_speed(
            tsdf,
            occupancy,
