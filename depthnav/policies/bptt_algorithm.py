...
class BPTT:
    ...
    def __init__(
        ...
        self.lambda_geo_aux = lambda_geo_aux
        ...
        try:
            ...
            for iter in tqdm(range(self.iterations)):
                ...
                for _ in range(self.horizon):
                    ...
                    if type(self.policy) == MlpPolicy:
                        ...
                        if self.policy.is_recurrent:
                            actions, latent_state, aux_outputs = self.policy(
                                obs, latent_state, return_aux=True
                            )
                        else:
                            actions, _, aux_outputs = self.policy(obs, return_aux=True)

                    if (
                        self.lambda_geo_aux > 0.0
                        and "geodesic_direction" in aux_outputs
                        and "geodesic" in obs
                    ):
                ...
                if iter % self.log_interval == 0:
                    ...
                    self._logger.record(
                        "train/geodesic_aux_loss", float(geodesic_aux_loss_mean)
                    )
